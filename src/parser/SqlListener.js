// Generated from Sql.g4 by ANTLR 4.13.1
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by SqlParser.
export default class SqlListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by SqlParser#fieldName.
	enterFieldName(ctx) {
	}

	// Exit a parse tree produced by SqlParser#fieldName.
	exitFieldName(ctx) {
	}


	// Enter a parse tree produced by SqlParser#id.
	enterId(ctx) {
	}

	// Exit a parse tree produced by SqlParser#id.
	exitId(ctx) {
	}


	// Enter a parse tree produced by SqlParser#value.
	enterValue(ctx) {
	}

	// Exit a parse tree produced by SqlParser#value.
	exitValue(ctx) {
	}


	// Enter a parse tree produced by SqlParser#notNUll.
	enterNotNUll(ctx) {
	}

	// Exit a parse tree produced by SqlParser#notNUll.
	exitNotNUll(ctx) {
	}


	// Enter a parse tree produced by SqlParser#size.
	enterSize(ctx) {
	}

	// Exit a parse tree produced by SqlParser#size.
	exitSize(ctx) {
	}


	// Enter a parse tree produced by SqlParser#unsigned.
	enterUnsigned(ctx) {
	}

	// Exit a parse tree produced by SqlParser#unsigned.
	exitUnsigned(ctx) {
	}


	// Enter a parse tree produced by SqlParser#typeRange.
	enterTypeRange(ctx) {
	}

	// Exit a parse tree produced by SqlParser#typeRange.
	exitTypeRange(ctx) {
	}


	// Enter a parse tree produced by SqlParser#autoIncrement.
	enterAutoIncrement(ctx) {
	}

	// Exit a parse tree produced by SqlParser#autoIncrement.
	exitAutoIncrement(ctx) {
	}


	// Enter a parse tree produced by SqlParser#primary.
	enterPrimary(ctx) {
	}

	// Exit a parse tree produced by SqlParser#primary.
	exitPrimary(ctx) {
	}


	// Enter a parse tree produced by SqlParser#unique.
	enterUnique(ctx) {
	}

	// Exit a parse tree produced by SqlParser#unique.
	exitUnique(ctx) {
	}


	// Enter a parse tree produced by SqlParser#key.
	enterKey(ctx) {
	}

	// Exit a parse tree produced by SqlParser#key.
	exitKey(ctx) {
	}


	// Enter a parse tree produced by SqlParser#indexKey.
	enterIndexKey(ctx) {
	}

	// Exit a parse tree produced by SqlParser#indexKey.
	exitIndexKey(ctx) {
	}


	// Enter a parse tree produced by SqlParser#defaultValue.
	enterDefaultValue(ctx) {
	}

	// Exit a parse tree produced by SqlParser#defaultValue.
	exitDefaultValue(ctx) {
	}


	// Enter a parse tree produced by SqlParser#comment.
	enterComment(ctx) {
	}

	// Exit a parse tree produced by SqlParser#comment.
	exitComment(ctx) {
	}


	// Enter a parse tree produced by SqlParser#property.
	enterProperty(ctx) {
	}

	// Exit a parse tree produced by SqlParser#property.
	exitProperty(ctx) {
	}


	// Enter a parse tree produced by SqlParser#fieldDefine.
	enterFieldDefine(ctx) {
	}

	// Exit a parse tree produced by SqlParser#fieldDefine.
	exitFieldDefine(ctx) {
	}


	// Enter a parse tree produced by SqlParser#create.
	enterCreate(ctx) {
	}

	// Exit a parse tree produced by SqlParser#create.
	exitCreate(ctx) {
	}


	// Enter a parse tree produced by SqlParser#table.
	enterTable(ctx) {
	}

	// Exit a parse tree produced by SqlParser#table.
	exitTable(ctx) {
	}


	// Enter a parse tree produced by SqlParser#iff.
	enterIff(ctx) {
	}

	// Exit a parse tree produced by SqlParser#iff.
	exitIff(ctx) {
	}


	// Enter a parse tree produced by SqlParser#not.
	enterNot(ctx) {
	}

	// Exit a parse tree produced by SqlParser#not.
	exitNot(ctx) {
	}


	// Enter a parse tree produced by SqlParser#exists.
	enterExists(ctx) {
	}

	// Exit a parse tree produced by SqlParser#exists.
	exitExists(ctx) {
	}


	// Enter a parse tree produced by SqlParser#ifNotExists.
	enterIfNotExists(ctx) {
	}

	// Exit a parse tree produced by SqlParser#ifNotExists.
	exitIfNotExists(ctx) {
	}


	// Enter a parse tree produced by SqlParser#createTable.
	enterCreateTable(ctx) {
	}

	// Exit a parse tree produced by SqlParser#createTable.
	exitCreateTable(ctx) {
	}


	// Enter a parse tree produced by SqlParser#statement.
	enterStatement(ctx) {
	}

	// Exit a parse tree produced by SqlParser#statement.
	exitStatement(ctx) {
	}


	// Enter a parse tree produced by SqlParser#options.
	enterOptions(ctx) {
	}

	// Exit a parse tree produced by SqlParser#options.
	exitOptions(ctx) {
	}


	// Enter a parse tree produced by SqlParser#keyType.
	enterKeyType(ctx) {
	}

	// Exit a parse tree produced by SqlParser#keyType.
	exitKeyType(ctx) {
	}


	// Enter a parse tree produced by SqlParser#keyName.
	enterKeyName(ctx) {
	}

	// Exit a parse tree produced by SqlParser#keyName.
	exitKeyName(ctx) {
	}


	// Enter a parse tree produced by SqlParser#index.
	enterIndex(ctx) {
	}

	// Exit a parse tree produced by SqlParser#index.
	exitIndex(ctx) {
	}


	// Enter a parse tree produced by SqlParser#other_stat.
	enterOther_stat(ctx) {
	}

	// Exit a parse tree produced by SqlParser#other_stat.
	exitOther_stat(ctx) {
	}


	// Enter a parse tree produced by SqlParser#init.
	enterInit(ctx) {
	}

	// Exit a parse tree produced by SqlParser#init.
	exitInit(ctx) {
	}



}